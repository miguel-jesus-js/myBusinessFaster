	...
	$.fn.paginate = function( options ) {
		...
		function createPaginationControls( pages ) {
			...
			if ( options.prev_next == true ) {
				obj.find( "." + options.pagination_class ).append( '<li class="prev page-item border radius-left"><a href="#" class="page-link">' + options.prev_text + '</a></li>' );
			}
			// For each div (slide) add a link in span for controls
			for ( var i = 0; i < pages; i++ ) {
				obj.find( "." + options.pagination_class ).append( '<li class="page-item border "><a class="page-link" href="#' + ( i + 1 ) + '">' + ( i + 1 ) + '</a></li>' );
			};

			// Add Next Button
			if ( options.prev_next == true ) {
				obj.find( "." + options.pagination_class ).append( '<li class="next page-item border radius-right"><a href="#" class="page-link">' + options.next_text + '</a></li>' );
			}
		}
